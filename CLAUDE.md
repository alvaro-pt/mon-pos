...
5. **Estado da caixa (máquina de estados)** — ✅ v1. Fonte única `POS.cash` (`cart.js`), **persistente por terminal** em `localStorage` (`pos_cash_<terminalId>`). Ver regras dedicadas em **§5.1**. Falta: ecrã/resumo Z imprimível, contagem por denominação, multi-TPA, histórico de turnos, permissões por perfil. Cliente: modal com pesquisa + formulário expansível (nome/NIF/morada/CP/localidade/país/telefone), com teclado on-screen.
